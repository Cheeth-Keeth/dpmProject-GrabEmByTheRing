...
public class Localizer {
	...
	static void fallingEdge(Odometer odometer) {
		...
		if (distance > DISTANCE) {
			...
			// delta is the angle of the real 90 axis when we use initial orientation as 0
			// axis
	...
	static void risingEdge(Odometer odometer) {
		...
		if (distance < DISTANCE) {
			...
			// turn counter-clockwise to find the back wall and the corresponding angle
			// alpha
			leftMotor.stop(true);
			rightMotor.stop(false);
			// reset the motor
			...
			// calculate the change in angle and then turn to the adjusted orientation
			// delta is the angle of the real 90 axis in the system where the original
			// heading was the zero axis.
		...
		// if the robot is actually facing away from the wall rather than facing the
		// wall, it will correct itself and use fallingEdge instead
		else {
	 ...
	 * orients the robot to close to the 0-axis.
	 * 
